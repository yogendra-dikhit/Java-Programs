...
/*
	public static int myAtoi(String s) {
		...
		for(String ss : s.split(" ")) {
			...
			if(Character.isAlphabetic(ss.charAt(0))) break;
			
			try {
				value = Integer.parseInt(ss);
				break;
			} catch (Exception e) {
				if(ss.length() > 1 && ((ss.charAt(0) == '+' && ss.charAt(1) == '-') || (ss.charAt(0) == '-' && ss.charAt(1) == '+') ))
				break;
				if(ss.charAt(0) == '+' && ss.length() != 1) value = Integer.MAX_VALUE;
				else if(ss.charAt(0) == '-' && ss.length() != 1) value = Integer.MIN_VALUE;
				else if(ss.contains(".") && ss.substring(0,ss.indexOf(".")).length()!=0)
					value = Integer.parseInt(ss.substring(0,ss.indexOf(".")));
